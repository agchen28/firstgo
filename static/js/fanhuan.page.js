...
showPages.prototype.createHtml = function(mode){ //生成html代码
	...
	switch (mode) {
		case 2 : //模式2 (前后缩略,页数,首页,前页,后页,尾页)
			if (prevPage < 1) {
			    strHtml += '<span class="page_start">上一页</span>';
			} else {
			    strHtml += '<a class="page_prev" href="javascript:void(0)" onclick="' + this.name + '.toPage(' + prevPage + ');">上一页</a>';
			}
			if (this.page != 1) strHtml += '<a href="javascript:void(0)" onclick="' + this.name + '.toPage(1);">1</a>';
				...
				if (i > 0) {
					if (i == this.page) {
						...
						if (i != 1 && i != this.pageCount) {
							strHtml += '<a href="javascript:void(0)" onclick="' + this.name + '.toPage(' + i + ');">' + i + '</a>';
						}
					}
				}
			}
			...
			if (this.page != this.pageCount) strHtml += '<a href="javascript:void(0)" onclick="' + this.name + '.toPage(' + this.pageCount + ');">' + this.pageCount + '</a>';
			if (nextPage > this.pageCount) {
			    strHtml += '<span class="page_end">下一页</span>';
			} else {
			    strHtml += '<a class="page_next" href="javascript:void(0)" onclick="' + this.name + '.toPage(' + nextPage + ');"><b>下一页</b><i></i></a>';
			}
			break;
...
showPages.prototype.printHtml = function (element) { //显示html代码
    if (this.pageCount > 1) {
        ...
        if (document.getElementsByClassName)
            pageNodeList = document.getElementsByClassName(element)
        else {
            var all = document.getElementsByTagName('*');
            for (var i = 0; i < all.length; i++)
                if (~(' ' + all[i].className + ' ').indexOf(' ' + element + ' '))
                    pageNodeList.push(all[i]);
        }

        for (var k = 0; k < pageNodeList.length; k++) pageNodeList[k].innerHTML = this.createHtml(2);
